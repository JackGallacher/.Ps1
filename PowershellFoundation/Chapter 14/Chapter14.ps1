...
#Task 1 -  Connect to another machine using an interactive 1:1 session, get services and exit the session
Enter-PSSession APPNUGGET
Get-Service
Exit-PSSession

#Task 2 - Run a command to view the most recent 10 system event logs across multiple servers using Invoke-Command
Invoke-Command -ScriptBlock {Get-EventLog System -Newest 10} -ComputerName FSNUGGET,APPNUGGET

#Task 3 - Create a new session to another machine, store a list of processes in a variable, disconnect from the session,
